    ...
    >
      ...
      >
        <StatsCard
          ...
          className="bg-linear-to-br from-green-50 to-emerald-50 border-success-lighter"
        />
        <StatsCard
          title="Total Orders"
          ...
          className="bg-linear-to-br from-blue-50 to-cyan-50 border-info-lighter"
      ...
      {((inventoryAlerts?.lowStock?.length || 0) > 0 ||
        ...
        <motion.div variants={itemVariants}>
          <Card className="border-error-lighter bg-error-lighter">
            <CardHeader>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2">
                {(inventoryAlerts?.outOfStock?.length || 0) > 0 && (
                  <div>
                    <h4 className="font-semibold text-error-dark mb-2 flex items-center gap-2">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    labelLine={false}
                    label={(props: any) =>
                      `${props.name || props.status || props.payload?.status || ""} ${(props.percent * 100).toFixed(0)}%`
                    }
                    outerRadius={80}
                    fill="#8884d8"
                    dataKey="count"
              ...
              <div className="space-y-3">
                {overviewData?.sales?.bestSellingProducts
                  ...
                  .map((product, index) => (
                    <div
                      key={product._id}
                      className="flex items-center justify-between p-3 rounded-lg bg-grey-100"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-info-lighter text-info-main flex items-center justify-center text-sm font-semibold">
                          #{index + 1}
                        </div>
                        <div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2">
                {(inventoryAlerts?.staleProducts?.length || 0) > 0 && (
                  <div>
                    <h4 className="font-semibold text-warning-dark mb-2 flex items-center gap-2">
                    ...
                    <div className="space-y-2 max-h-32 overflow-y-auto">
                      {inventoryAlerts?.staleProducts
                        ...
                        .map((product) => (
                          <div
                            key={product._id}
                            className="flex items-center justify-between p-2 bg-warning-lighter rounded border"
                          >
                            <div>
                              <p className="font-medium text-sm">
                                {product.name}
                ...
                {(inventoryAlerts?.noSalesProducts?.length || 0) > 0 && (
                  <div>
                    <h4 className="font-semibold text-grey-700 mb-2 flex items-center gap-2">
                      <XCircle className="w-4 h-4" />
                    ...
                    <div className="space-y-2 max-h-32 overflow-y-auto">
                      {inventoryAlerts?.noSalesProducts
                        ...
                        .map((product) => (
                          <div
                            key={product._id}
                            className="flex items-center justify-between p-2 bg-grey-100 rounded border"
                          >
                            <div>
                              <p className="font-medium text-sm">
                                {product.name}
